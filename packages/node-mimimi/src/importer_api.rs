...
impl ImporterApi {
    ...
    #[napi]
    pub async fn get_import_status(&self) -> napi::Result<Option<ImportStatus>> {
        if let Ok(import_mail_state) = self.importer.essentials.load_remote_state().await {
            let import_status = ImportStatus::try_from(import_mail_state.status);
            Ok(import_status.ok())
        } else {
            Ok(None)
        }
...
impl ImporterApi {
    fn spawn_importer_task(&mut self) -> napi::tokio::task::JoinHandle<()> {
        ...
        napi::tokio::task::spawn(async move {
            ...
            if let (Some(error_handler), Err(error_to_send)) = (error_handler, import_res) {let call_status = error_handler.call(
