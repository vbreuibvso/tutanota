...
export class GlobalSettingsViewer implements UpdatableSettingsViewer {
	...
	private deleteCustomDomain(domainInfo: DomainInfo) {
		Dialog.confirm(
			lang.getResolved("confirmCustomDomainDeletion_msg", {
					...
					.catch(
						ofClass(PreconditionFailedError, () => {
							...
							if (registrationDomains.indexOf(domainInfo.domain) !== -1) {
								Dialog.message(
									lang.getResolved("customDomainDeletePreconditionWhitelabelFailed_msg", {
										"{domainName}": domainInfo.domain,
									}),
								)
							} else {
								Dialog.message(
									lang.getResolved("customDomainDeletePreconditionFailed_msg", {
										"{domainName}": domainInfo.domain,
									}),
								)
							}
