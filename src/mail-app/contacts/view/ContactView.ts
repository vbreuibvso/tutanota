...
export class ContactView extends BaseTopLevelView implements TopLevelView<ContactViewAttrs> {
					...
					m(FolderColumnView, {
						...
						content: [
							m(
								SidebarSection,
								{
									name: lang.makeResolved("group_info", getGroupInfoDisplayName(locator.logins.getUserController().userGroupInfo)),
								},
	...
	private renderContactListRow(contactListInfo: ContactListInfo, shared: boolean) {
		...
		return m(SidebarSectionRow, {
			icon: Icons.People,
			label: lang.makeResolved("contactlist_name", contactListInfo.name),
