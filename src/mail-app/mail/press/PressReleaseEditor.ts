...
export function openPressReleaseEditor(mailboxDetails: MailboxDetail): void {
	...
	async function send() {
		...
		const choice = await Dialog.choice(
			lang.makeResolved("press_release_confirmation", `Really send the press release out to ${recipients.length} recipients?`),
			[
				{
					text: "cancel_action",
					value: "cancel",
				...
				{
					text: lang.makeResolved("noOp_action", "Just test"),
					value: "test",
				},
				{
					text: "yes_label",
		...
		for (let recipient of recipients) {
			...
			try {
				...
				Dialog.message(lang.makeResolved("error_msg", `Error sending to ${recipient.email}: ${e.message}.\nStopping.`))
				didFinish = false
				break
			}
		}
	...
	const header: DialogHeaderBarAttrs = {
		...
		middle: lang.makeResolved("pr", "Press Release"),
		right: [
			{
				label: "send_action",
				click: send,
...
function getValidRecipients(recipientsJSON: string): Array<PressContact> {
	...
	try {
		...
		throw new UserError(lang.makeResolved("parse_json", "Unable to parse recipients JSON:\n" + e.toString()))
	}

	if (!(parsed instanceof Array)) {
		throw new UserError(lang.makeResolved("rec_arr", "Recipients must be an array"))
	}

	return parsed.map(({ email, greeting }) => {
		if (typeof email !== "string" || !isMailAddress(email, false)) {
			throw new UserError(lang.makeResolved("no_email", `Not all provided recipients have an "email" field`))
		}

		if (typeof greeting !== "string") {
			throw new UserError(lang.makeResolved("no_greeting", `Not all provided recipients have a "greeting" field`))
		}
