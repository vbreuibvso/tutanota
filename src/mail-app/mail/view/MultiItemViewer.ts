...
import { lang, ResolvedTranslation, TranslationText } from "../../../common/misc/LanguageViewModel"
...
export type MultiItemViewerAttrs<T> = {
	...
	loadAll: () => unknown
	stopLoadAll: () => unknown
	getSelectionMessage: (entities: ReadonlyArray<T>) => TranslationText
}
...
export function getMailSelectionMessage(selectedEntities: ReadonlyArray<Mail>): ResolvedTranslation {
	let nbrOfSelectedMails = selectedEntities.length

	if (nbrOfSelectedMails === 0) {
		return lang.getResolved("noMail_msg")
	} else if (nbrOfSelectedMails === 1) {
		return lang.getResolved("oneMailSelected_msg")
	} else {
		return lang.getResolved("nbrOfMailsSelected_msg", {
