...
export class RichTextToolbar implements Component<RichTextToolbarAttrs> {
	...
	private renderToggleButton(title: TranslationText, icon: Icons, click: () => void, isSelected: () => boolean): Children {
				...
				createDropdown({
					lazyButtons: () =>
						numberRange(8, 144).map((n) => {
							return {
								label: lang.makeResolved("font_size_" + n, n.toString()),
								click: () => {
