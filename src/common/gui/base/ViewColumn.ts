...
export class ViewColumn implements Component<Attrs> {
	...
	private readonly headerCenter: MaybeLazy<TranslationText>
	...
	constructor(
		...
		columnType: ColumnType,
		{
			...
			headerCenter,
			ariaLabel = () => this.getTitle().text,
		}: {
			minWidth: number
			maxWidth: number
			headerCenter?: MaybeLazy<TranslationText>
	...
	view() {
		...
		const landmark = this.ariaRole ? landmarkAttrs(this.ariaRole, this.ariaLabel ? this.ariaLabel() : this.getTitle().text) : {}
		return m(
	...
	getTitle(): ResolvedTranslation {
		return lang.getMaybeLazy(resolveMaybeLazy(this.headerCenter))
	}
